...
*/				    
	...
	public void visit(Division n) {
		...
		String op2 = getExpr(n.e2, false);
		printInstr("popq", new String[]{"%r10"});
		//printInstr("popq", new String[]{"%rax"});
		op1 = "%r10";

		printInstr("idivq", new String[]{op1, op2});
		*/
		
		String restLabel = LabelCreator.getInstance().getCustomLabel("divRest");
		String restName = LabelCreator.getInstance().getCustomName();

		String doDivLabel = LabelCreator.getInstance().getCustomLabel("diDiv");
		String doDivName = LabelCreator.getInstance().getCustomName();

		String noDivLabel = LabelCreator.getInstance().getCustomLabel("noDiv");
		String noDivName = LabelCreator.getInstance().getCustomName();

		// check for 0
		String rhs = getExpr(n.e2, false); // bottom
		printInstr("movq", new String[]{"%rax", "%r9"});
		printInstr("cmpq", new String[]{"$0", "%r9"});
		printInstr("jz", new String[]{noDivName});
		printInstr("jmp", new String[]{doDivName});
		
		System.out.println(doDivLabel);
		printInstr("movq", new String[]{"%rax","%r9"});
		String lhs = getExpr(n.e1, false); // top
		printInstr("cqo");
		printInstr("idivq", new String[]{"%r9"});
		printInstr("jmp", new String[]{restName});
		System.out.println(noDivLabel);
		printInstr("movq", new String[]{"$0", "%rax"});
		printInstr("jmp", new String[]{restName});
		System.out.println(restLabel);
